import type { ToolRegistry, ToolSpec } from "@seekforge/tools";
...
export function toolPromptHintLines(source?: ToolPromptHintSource): string[] {
  ...
  if (has("install_skill")) {
    lines.push("Use install_skill when the user asks to install, create, or save a SeekForge skill; it writes only to the global user skill directory.");
    lines.push("Install SeekForge skills with install_skill, not write_file/apply_patch/exec_shell, and never create workspace .seekforge/skills unless the user explicitly asks to edit project files.");
  }
  if (has("tool_search")) {

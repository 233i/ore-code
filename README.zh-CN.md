...
<p align="center">
  面向长上下文编码、结构化工具调用、本地项目上下文、DeepSeek / Mimo provider、MCP 集成、技能、自动化，以及 macOS 和 Windows 桌面体验。
</p>
...
- DeepSeek-first Agent runtime，同时支持 Mimo 和自定义 OpenAI-compatible provider。
...
- 在 `~/.ore-code/config.toml` 中配置 DeepSeek、Mimo 或自定义 endpoint；API Key 只进入系统钥匙串。

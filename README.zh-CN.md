...
- DeepSeek-first Agent runtime，同时支持 Mimo、Ark Coding 和自定义 OpenAI-compatible provider。
...
- 在 `~/.ore-code/config.toml` 中配置 DeepSeek、Mimo、Ark Coding 或自定义 endpoint；API Key 只进入系统钥匙串。
